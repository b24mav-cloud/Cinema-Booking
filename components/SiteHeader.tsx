...
export function SiteHeader({ admin = false }: { admin?: boolean }) {
  ...
  return <header className="site-header"><nav className="nav shell">
    <Link className="brand text-decoration-none" href="/"><img className="brand-mark" src="/cinema-logo.svg" alt="CinemaBooking" width={64} height={64} /><span>Cinema<span className="gold">Booking</span>{admin ? " Admin" : ""}</span></Link>
