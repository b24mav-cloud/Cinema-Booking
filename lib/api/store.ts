...
import type { Movie, Seat, Showtime, Store } from "../types";
...
const seed = (): Store => {
  ...
  const showtimes: Showtime[] = [
    { id: 1, movieId: 1, experience: "IMAX", price: 520, startTime: new Date(tomorrow.getTime() + 18 * 3600000).toISOString(), auditorium: "Auditorium 1", auditoriumType: "regular", seats: makeSeats(1).map(s => ({ ...s, price: 520 })) },
    { id: 2, movieId: 2, experience: "Director's Cut", price: 450, startTime: new Date(tomorrow.getTime() + (20 * 60 + 30) * 60000).toISOString(), auditorium: "Auditorium 2", auditoriumType: "regular", seats: makeSeats(2).map(s => ({ ...s, price: 450 })) },
...
export async function readStore(): Promise<Store> {
  ...
  memory.showtimes = memory.showtimes.map((showtime, index) => {
    ...
    const next: Showtime = { ...showtime, auditoriumType: showtime.auditoriumType ?? (showtime.auditorium?.toLowerCase().includes("vip") ? "vip" : "regular"), experience: showtime.experience ?? (showtime.movieId === 1 ? "IMAX" : "Director's Cut"), price, seats: showtime.seats.map(seat => ({ ...seat, price })) };
    changed ||= showtime.price !== price || !showtime.auditoriumType;
    return next;
  });
